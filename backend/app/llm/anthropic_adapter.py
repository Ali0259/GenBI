"""
app.llm.anthropic_adapter
--------------------------
Concrete BaseLLMAdapter implementation backed by the Anthropic Messages API.
"""
...
class AnthropicAdapter(BaseLLMAdapter):
    """
    Talks to the Anthropic /v1/messages endpoint. Requires an API key.
    Uses raw httpx (rather than the anthropic-python SDK) to keep the
    backend image's dependency footprint small and its timeout/retry
    ...
    async def _post_message(self, system_prompt: str, user_prompt: str) -> LLMCompletionResult:
        if not self.api_key:
            raise LLMAdapterError(self.provider, "No API key configured for Anthropic adapter.")
        ...
        try:
            ...
            raise LLMAdapterError(self.provider, "Unexpected response shape from Anthropic.", exc) from exc
    ...
    async def generate_sql(self, natural_language_question: str, schema_context: str,
        ...
        return await self._post_message(system_prompt, natural_language_question)
    ...
    async def repair_sql(self, failed_sql: str, database_error_message: str, schema_context: str,
        ...
        return await self._post_message(system_prompt, "Repair the SQL statement above.")

    async def health_check(self) -> bool:
        try:
            await asyncio.wait_for(
                self._post_message("You are a health check probe.", "Reply with the word OK."),
